...
def parse_response(response):
    ...
    for line in lines:
        if ':' in line:
            ...
            if keyword_line:  # Check if keyword_line is not empty
                try:
                    keyword = keyword_line.split(': ', 1)[1]
                    slides_content.append(keyword)
                except IndexError:
                    print("IndexError: Could not extract keyword from line:", keyword_line)
                    # Handle the error appropriately, such as skipping this line or logging the error
